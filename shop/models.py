...
from todianedev.storage import SecureStorage, PublicStorage
...
class ProductDownload(models.Model):
    """
    Flexible download model - allows any file type with custom button labels.
    Replaces the old windows_file/mac_file/pdf_file approach.
    """
    ...
    label = models.CharField(
        max_length=100,
        default="Add To Cart",
        help_text="Button text shown to the customer (e.g. 'Download PDF', 'Windows Version', 'Starter Pack')",
    )
...
class OrderItem(models.Model):
    ...
    def get_download_url(self):
        """
        Download URLs are always served via secure_download.
    ...
    def has_downloadable_content(self):
        ...
        return self.product.downloads.exists()
...
class Purchase(models.Model):
    ...
    def __str__(self):
        return f"Review by {self.user.display_name} on {self.product.title}"
...
class DownloadLog(models.Model):
    ...
    def __str__(self):
        return f"{self.user} → {self.order_item} at {self.downloaded_at}"